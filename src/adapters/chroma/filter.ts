...
/**
 * Translate Victor's unified filter format to ChromaDB's native filter format.
 *
 * ChromaDB uses MongoDB-style operators natively (same as Pinecone),
 * so this is largely a passthrough with normalization of shorthand values.
 *
 ...
 *
 * // Output (ChromaDB format — same structure):
 * { genre: { $eq: "drama" }, year: { $gte: 2020 } }
 * ```
 */
